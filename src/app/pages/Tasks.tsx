...
export function Tasks() {
  ...
  const fetchTasks = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/teams/${currentTeamId}/tasks`, getAuthHeaders());
  ...
  useEffect(() => { fetchTasks(); }, [currentTeamId]);

  const handleAddTask = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    const params: CreateTaskParams = { task_name: newTitle, due_date: newDueDate };
    try {
      await axios.post(`${API_BASE_URL}/api/teams/${currentTeamId}/tasks`, params, getAuthHeaders());
      await fetchTasks();
      setNewTitle("");
      setNewDueDate("");
      ...
      const fakeNewTask: Task = {
        id: Date.now(), team_id: Number(currentTeamId), assignee_id: 0,
        assignee_name: currentUser, task_name: params.task_name,
        due_date: params.due_date, status: "pending",
  ...
  const handleToggleStatus = async (id: number, currentStatus: string) => {
    ...
    const patchBody = { task_name: targetTask.task_name, due_date: targetTask.due_date, status: nextStatus };
    try {
      await axios.patch(`${API_BASE_URL}/api/tasks/${id}`, patchBody, getAuthHeaders());
    } catch { /* apply optimistically */ }
    setTasks(prev => prev.map(t => t.id === id ? { ...t, status: nextStatus } : t));
  };

  const displayedTasks = filterMyTasks ? tasks.filter(t => t.assignee_name === currentUser) : tasks;
        ...
        <div className="p-6 border-b border-blue-100 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={() => setFilterMyTasks(!filterMyTasks)}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${filterMyTasks ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
            >
              <Filter className="w-4 h-4 inline mr-2" />내 업무
            </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {isLoading ? (
                <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-500">로드 중...</td></tr>
              ) : displayedTasks.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-500">업무가 없습니다.</td></tr>
              ) : (
                  ...
                  <tr key={task.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <button onClick={() => handleToggleStatus(task.id, task.status)}
                        className={`inline-block px-3 py-1 rounded-full text-xs font-semibold transition-all cursor-pointer hover:shadow-md ${getStatusColor(task.status)}`}
                      >
